...
Calibrated (session 58): the competitor maximum is a Gumbel variable in all five models, its mean grows as sqrt(2 ln m), and its two fitted parameters predict the detection curve of an injected write within 0.1, so the instrument's resolution is 0.25-0.4 of the state's norm (e483). For a rotated dictionary the scale of the law is exactly the atoms' second moment; for the native one it is 2.9-6.2 times that. That provenance factor is the extreme-value form of the own-word edge: 6-14 at the first block, falling with depth to 3.5-5, and rising through Pythia's training from 2.2-3.0 at step 1000 (e492).
