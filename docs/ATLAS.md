# Atlas of the WDD program: 481 experiments by technical area
...
- what was asked;
- the twenty-three technical areas;
...
- **Scale.** 481 experiment scripts (e00-e516 plus variants), 72 sessions, 2026-09-19 to 2026-09-25, one GPU. Most runs take 1-5 minutes.
- **Status counts** (from [`atlas/index.tsv`](atlas/index.tsv)): 127 established, 108 supported, 63 narrowed, 28 mixed, 124 refuted, 12 superseded, 8 null, 4 retracted, 7 tools.
...
- **Phase 3** (sessions 33-72, e374-e516): readers and readouts; then self-description, meaning how well a model's own write rows describe its own states compared with controls (its "native vocabulary"). Also a 7B workspace agenda, the huge directions and sinks, a backcast "vision" round that trained models from scratch, causal tests of concept words and of a re-implemented block (session 46), WDD as a forensic instrument (sessions 47-48), the learning signal in native coordinates (session 49), whether the native words have a grammar (session 50), whether a description is a self-sufficient state (session 51), native words as interchange coordinates (sessions 52-53), the life cycle of native words (session 54), WDD applied to its own objects: fixed points, word algebra, role typing and induced geometry (session 55), the audits of its two open results (session 56), WDD put to WorkspaceBench's problems as a workspace reader (session 57), the instrument calibrated against classical theory (session 58), the provenance factor decomposed with a bridge to SAE features (session 59), and that bridge followed across depth and into activation, with the vocabulary's two clocks (session 60), the control that shows a feature is state-like, not write-like, in native words (session 61), and the follow-ups: the two clocks' anatomy in Pythia and OLMo, a second SAE family, and the kind of atom that becomes a word (session 62), what predicts whether a feature follows its writers, with the extreme-value account of vocabulary formation written into `THEORY.md` (session 63), and that account tested state by state and corrected: the floor stands, and the numerator is the centred chord of a position's largest writes (session 64), a partial predictor of the whole projection profile, with the crowd of small writes amplifying it early in training and contracting it late (session 65); the clean maximum is extreme-value selection over a nearly exchangeable residual, and the contraction is the MLPs' response to the chord, area 05's learned contraction, which spares the chord relative to any other direction (session 66); the contraction is isotropic and blind to the atoms in the linear regime, and the chord's sparing is a nonlinear, chord-amplitude effect (session 67) specific to the position's own writes, while a dense checkpoint sweep shows wordhood turning on as a fast crossover with sparsification before it and the contraction after (session 68), and, below step 512, the state cloud's collapse and re-expansion before sparsification, with the gradient spread rather than concentrated (session 69); and the rows that end as words have no lineage before sparsification, their selectivity leading their usage by a doubling and the early vocabulary being other rows (session 70); and the vocabulary is replaced in identity at the churn rate of the large rows, under a boundary that drifts toward larger, sparser rows and then holds (session 71); and that boundary is the extreme-value floor, which predicts the next checkpoint's entrants out of sample (session 72).
- **Terms.**
...
flowchart TD
  ...
  A20 --> A22["22 Model tested"]
  A22 --> A23["23 Training sweep"]
  A05 --> A22
...
- What is it as an instrument, calibrated against classical theory (20)?
- What is a learned SAE feature made of, in the model's own words (21)?
...
18. WDD applied to itself adds little, but native handles compose.
...
20. Calibrated against classical theory, WDD is a detector with a known threshold law.
    ...
    - The provenance factor is write sparsity on an accent: the largest writes standing out (rising through training) over a collective alignment present from step 512 (e493); across 45 checkpoint-block cells the writers' part tracks write sparsity and the non-writers' part the state cloud's second-order alignment, and neither predicts the other (e498). OLMo shows the same two clocks, the accent decaying after its early peak. MLP rows are the words at every checkpoint, though attention comes to carry most of the state's energy (e500). [20]

21. A learned SAE feature has a state's provenance profile, not a write's.
    - Its top native word is an MLP row for 76-93% of features at every depth, from any block; a quarter of the most used rows are features; the frequent features are the nearest to single rows (e494, e495).
    - It is as sparse in native words as a whole state, far sparser than a random or covariance-matched direction (e497); on an independently trained TopK family it sits between covariance-matched directions and states (e499).
    - Its firing follows its top row's activity partly, AUC 0.62 on the ReLU family and 0.73 on the TopK family; the eight-word ledger reaches 0.83 on the TopK family and adds nothing on the ReLU one (e496, e499). The TopK lead is the features' own sparsity: at matched frequency the families are alike, and the family label carries nothing (e501); the other rows add modestly where features are sparse, and the non-MLP part of the state carries the firing (e502, e504). [21]

22. The extreme-value model, tested state by state, keeps its floor and loses its numerator.
    - The Gumbel floor is right for provenance-free atoms in every model, block and checkpoint (rotated calibration 0.97-1.00; e503, e506).
    - A word's projection is its own coefficient only in part (0.3-0.7); the position's other large writes supply the rest through their cosines, the crowd of small writes subtracts, and the top non-writer atom is the chord's shadow (GPT-2, OLMo) or attention's (Pythia); the non-writers' excess is a tail, not second-order alignment (1.02-1.24 of 1.3-2.3) (e503, e505).
    - The largest writes summed as written overshoot the maximum twofold where they are as large as the state, because of the common direction they share; centred, the chord predicts GPT-2's level within 1-13% and names the atom in a third to a half of states, the crowd's contraction mattering in OLMo and the small writes in Pythia; over Pythia's training the carrier moves from the dense sum on head atoms (the accent, step 512) to the sparse chord (the words) (e505, e506). [22]
    - Against the whole profile the centred chord is a partial predictor (GPT-2: calibrated within 13-25%, the largest atom in 32-46% of states, the true top hundred ordered at 0.3-0.4), and the crowd's gain along it runs from +1.4 at Pythia step 512 to -0.64 at the end while attention's rises from nothing to 0.3-0.5 (e507). [22]
    - The chord's clean maximum in GPT-2 is extreme-value selection over a nearly exchangeable residual (the null reproduces the level within 2% and the winner's rank), while OLMo's residual contracts the chord's extremes and Pythia's mid-training residual reinforces them (e508); the crowd's contraction is the MLPs' response to the chord (1.2-1.9 times the actual gain), born with area 05's learned contraction, and a random direction of the chord's size is contracted two to three times more: the contraction spares the chord (e509), but not the atoms: as a linear response it is isotropic within 25% and native rows are contracted slightly more than random directions; the chord's sparing is a nonlinear effect at chord amplitude, present in GPT-2 and late Pythia and reversed in OLMo (e510). [22]
    - The sparing is the position's own writes: a fake chord of native rows with the same coefficients and norm, even of the most-used rows, is contracted like a random direction (e511). [22]

23. Wordhood turns on in training as a fast crossover with an order, not a discontinuity.
    - At 18 Pythia checkpoints half of the writer-win rate's rise falls between steps 2000 and 8000, the most concentrated change of nine observables; sparsification precedes it (midpoint near step 1500-1700), the crowd's gain and the contraction follow (2400-15400), the chord's amplitude grows last (14000-19000); the accent is in place before step 512; the state's late attention phase erodes the last block's words (e512). [23]
    - Below step 512: a transient gradient concentration in the first 16 steps, the state cloud's collapse to 9-16 dimensions at steps 16-64 with attention's share collapsing and the accent born, a plateau to step 256, then sparsification as every row turns selective while the cloud re-expands and attention returns; the gradient is spread when the writes sparsify (e513). [23]
    - The rows that end as words are indistinguishable from magnitude-matched non-words through step 1000 (against the population they are the larger rows from step 1000); selectivity and write kurtosis separate at steps 2000-3000, one doubling before usage, and grow with it to the end; gradient share never separates and direction only at step 64000; before step 2000 the future words are used less than their controls; the early wins of e512-e513 were the embedding, the maxima of steps 512-2000 head atoms, the rise from 2000 MLP writers (e514). [23]
    - The vocabulary is replaced in identity (a twentieth of the step-1000 words survive; seven-tenths of the end's words are new after step 4000) at the rate the 256 largest rows churn, under a boundary that through steps 256-16000 admits rows more selective and larger than those it releases and then holds; the words are never the largest rows (e515, e515b). [23]
    - The boundary is the floor: a non-word row's largest projection over the calibrated Gumbel floor at a checkpoint picks the next checkpoint's entrants at AUC 0.76-0.97, ahead of magnitude, selectivity and kurtosis (0.5-0.7) and level with usage; the own write almost never clears the floor and governs staying instead (0.90-0.99) (e516, e516b). [23]
...
- **Narrative.**
  ...
  - [`GRAPH.md`](GRAPH.md): hypotheses H0-H357 with their tests and fates.
  - [`KILLED.md`](KILLED.md): every kill and retraction.
  - [`THEORY.md`](THEORY.md): transport theory, the self-description account, (section 3m) the extreme-value account of why sparse writers become words, and (section 3n) its test and correction: the chord and the crowd.
...
- **Sessions to experiments.**
  ...
  - Phase 3:
    ...
    - S40 e415-e420; S41 e421-e422; S42 e421b, e423-e426; S43 e427-e431; S44 e432-e443 with e392b and e437b; S45 e444-e450; S46 e451-e455 with e452b; S47 e456-e459; S48 e460-e461; S49 e462; S50 e463-e464; S51 e465; S52 e466-e469; S53 e470; S54 e471; S55 e472-e475; S56 e476-e477; S57 e478-e482 with e478b; S58 e483-e492; S59 e493-e494; S60 e495-e496; S61 e497; S62 e498-e500; S63 e501; S64 e502-e506; S65 e507; S66 e508-e509; S67 e510; S68 e511-e512; S69 e513; S70 e514; S71 e515, e515b; S72 e516, e516b.
