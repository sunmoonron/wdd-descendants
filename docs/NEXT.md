...
Each item gives the question, where it comes from, a design at the program's usual scale (a few minutes on one A100 unless marked), the controls, and what would count as an answer. The ids B1-B16 are backlog labels, not experiment numbers; the next experiment is e517. B1 and B3 were run in session 56 (e476, e477); their entries say what came out.
...
**Session 68 leaves (e511, e512):** (a) the removal-addition asymmetry of e511: add a copy of the real chord (double it) beside removing it, to separate "the MLPs expect this component" from "removal of a present component"; (b) the synthetic-task test proposed in the relayed list: a tiny transformer trained for two minutes on modular addition, copying or induction, then the write-sparsity, chord and extreme-value statistics, to see whether the phenomenon is a property of residual-stream computation or of language; (c) what precedes sparsification, since it precedes wordhood: the early attention takeover (0.14 to 0.49 of the state by step 2000) is the candidate, testable on checkpoints below step 1000.
...
**Session 72 leaves (e516, e516b):** (a) the synthetic-task test, now with the selection criterion to transplant as well: a tiny transformer whose algorithm is known, trained with checkpoints, to see whether its words are selected by the same floor; (b) the floor's own drift: the entrants' rising magnitude (e515b) should be the floor rising with the states' geometry, testable by tracking the floor level and the entrants' projections over it across checkpoints; (c) the asymmetry of entry (the chord carries the row across) and staying (the own write holds it), on GPT-2 and OLMo at the end of training, where no checkpoints exist but entry and exit across evaluation texts can stand in.
