...
- H349 The chord's sparing is a property of the vocabulary's manifold (any chord of native rows). → e511 → KILLED: a chord of native rows with the position's own coefficients and norm, random or the block's most-used, is contracted like a random direction in GPT-2 and Pythia at step 16000 and 0.6-0.85 of the way to it in OLMo and Pythia at the end; only the rows that wrote at the position are spared. The sparing is write history.
- H350 The onset of wordhood is a sharp transition in training. → e512 → NARROWED: a fast crossover. Half of the writer-win rate's total rise falls in the two doublings from step 2000 to 8000 (Pythia block 12: 0.09 to 0.43), the most concentrated of nine observables, then four doublings creep to 0.73; the attention share is the least concentrated. Sparsification (effective writes, midpoint near step 1500-1700) precedes wordhood (2800-6600), which precedes the contraction (2400-15400) and the chord's growth (14000-19000), at every block. The state's energy moves in two phases, and the late attention phase erodes the last block's words (0.70 to 0.48).
- H351 Sparsification is initiated by the gradient concentrating on a few MLP rows. → e513 → KILLED: the gradient is most concentrated in the first 32 steps (participation ratio 575 of 4096, before the state cloud's collapse) and is spread over 2400-2900 rows by the time the writes sparsify (steps 256-2000); sparsification is every row turning selective (median activity 0.50 to 0.19, no row dying) while the cloud re-expands (16 to 133 dimensions) and attention takes the state back (0.06 to 0.49). What precedes it is the collapse and re-expansion of the state cloud, with the accent born at the collapse (tail factor 1.00 through step 32, 1.14-1.48 at step 64).
- H352 The rows that end as native words have an identifiable lineage before wordhood (selective, then aligned, then words). → e514 → NARROWED to a faint precursor: against non-word rows of the same block and eventual magnitude nothing tells the future words apart through step 1000 (AUC 0.42-0.58 for selectivity, kurtosis, gradient share, direction, magnitude), while against random rows they are larger and more reinforced from step 1000 (magnitude AUC 0.64, gradient share 0.59), two doublings before they are words, and never more selective (0.44-0.55); among rows of one size selectivity and write kurtosis cross AUC 0.6 at steps 2000-3000, one doubling before usage does (3000-8000), and grow with it to the end (0.75-0.79 against usage 1.00); gradient share never separates among rows of one size, direction only at step 64000. Before step 2000 the future words are used less than their controls: the early vocabulary is other rows.
- H353 The early writer wins of e512-e513 are MLP rows. → e514 → KILLED: they are the token embedding (0.37-0.55 of states at block 6 through step 16, none after step 64); the maxima of steps 512-2000 are head atoms mostly; the rise from step 2000 is MLP writers (0.09 to 0.73 at block 12). The order of e512 stands for the rise.
