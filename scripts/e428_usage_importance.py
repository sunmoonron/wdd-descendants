...
mag = torch.zeros(NBk, DFF, device=DEV); tay = torch.zeros(NBk, DFF, device=DEV); wn = torch.stack([arch.wdir(b).norm(dim=-1) for b in range(NBk)])
...
for s0 in range(0, sc_ids.shape[0], 2):
    ...
    for b in range(NBk):
        ...
        mag[b] += h.abs().sum((0, 1)) * wn[b]; tay[b] += (h * gr).sum((0, 1))
...
scores = dict(usage=usage.flatten(), magnitude=mag.flatten(), taylor=tay.flatten(), weight=wn.flatten(), random=torch.rand(NBk * DFF, generator=g).to(DEV))
...
for N in (16, 64, 256):
    ...
    res["overlap"][str(N)] = {k: len(set(tops["usage"].tolist()) & set(tops[k].tolist())) / N for k in ("magnitude", "taylor", "weight")}
...
log(summ); record(f"e428_usage_{name}", res, summ)
