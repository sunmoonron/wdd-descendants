"""e188: shape of provenance decay with depth. For dominant block-b writes (b = 0..4, typical states), the raw
...
for b in range(0, min(5, NL - 2)):
    ...
    for lv in range(b, NL):                                                                        # H[b] = pre-write state, H[b+1] = after block b
        H = c.s["H"][lv][ids].float().to(DEV); typ = typical_mask(H); m = typ & big
...
record(f"e188_survdist_{tag}", dict(model=tag, per_birth=out), f"median drop at +1: {np.median(first):+.2f}, at +2: {np.median(second):+.2f}, mean later drops: {np.median(later):+.3f} | profiles: " + " || ".join(f"b{b}: pre {out[b]['profile'][-1]:+.2f} " + " ".join(f"{out[b]['profile'][k]:.2f}" for k in sorted(out[b]['profile']) if k >= 0) for b in out))
