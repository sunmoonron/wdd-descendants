"""S1d (GPU): REINFORCING partners (negative rho here means cosine +|rho| with the write, coefficient of the same sign): the largest write is accompanied by P correlated true writers. One-shot should pick them all; OMP should explain them away. The largest write loses a fraction f=0.8 of its
...
rows = []; m = 64 * d
for P, rho, in_dict, K in itertools.product((1, 4, 16, 64), (-0.8, -0.5, -0.2), (True, False), (16, 64)):
    ...
    coef = (f * c[:, 0] / (P * abs(rho)))[:, None].expand(N, P)                                        # each partner removes f*c/P along dtop
...
json.dump(dict(rows=rows, f=f), open(f"{RES}/s1d_reinforcers.json", "w"), indent=1)
with open(f"{RES}/FINDINGS.log", "a") as fh: fh.write(time.strftime("%Y-%m-%d %H:%M:%S") + " s1d_reinforcers: " + " | ".join(f"P{r['P']} rho{r['rho']} {'dict' if r['in_dict'] else 'nodict'} K{r['K']}: omp {r['recall_omp']:.2f} os {r['recall_oneshot']:.2f} prom {r['prom_med']:.2f}" for r in rows) + "\n")
