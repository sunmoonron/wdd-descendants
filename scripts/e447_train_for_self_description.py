...
EA = torch.load("/workspace/wdd/eval_ids_all.pt", weights_only=False)[name]; tr = EA["cen_ids"].to(DEV); te = EA["eval_ids"][:6].to(DEV); del EA
def train_words(m):
    h = m.transformer.h
...
summ = (f"gpt2 lambda {lam}: held-out LM {res['test_lm_old']:.3f} -> {res['test_lm_new']:.3f} | k4/k16 own {r_('own', 4):.2f}/{r_('own', 16):.2f} rot {r_('rot', 4):.2f}/{r_('rot', 16):.2f} covA {r_('covA', 4):.2f}/{r_('covA', 16):.2f} "
...
log(summ); record(f"e447_trainsd_gpt2_lam{lam}", res, summ)
