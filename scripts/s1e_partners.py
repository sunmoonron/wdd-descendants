"""S1e (GPU, replaces S1c/S1d whose in-dictionary partners overwrote shared atoms): the largest write is
...
rows = []; m = 64 * d
for P, rho, f, in_dict in itertools.product((1, 8, 32), (-0.8, -0.5, -0.2, 0.2, 0.5), (0.8,), (True, False)):
...
json.dump(dict(rows=rows), open(f"{RES}/s1e_partners.json", "w"), indent=1)
with open(f"{RES}/FINDINGS.log", "a") as fh: fh.write(time.strftime("%Y-%m-%d %H:%M:%S") + " s1e_partners: " + " | ".join(f"P{r['P']} rho{r['rho']} {'dict' if r['in_dict'] else 'nodict'}: omp {r['recall_omp']:.2f} os {r['recall_oneshot']:.2f} prom {r['prom_med']:.2f}" for r in rows) + "\n")
