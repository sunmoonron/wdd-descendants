...
X_all = subject_states(items, layers)
...
for L in layers:
    ...
    S = dict(lens=lens_scores(X, gain, WU, rms), lens_centred=lens_scores(Xc, gain, WU, rms), pca16=pooled_word_scores(parts, torch.ones(N, parts.shape[1], device=DEV), gain, WU, rms),
    ...
    for rd, Xh in [("native16", mu + torch.einsum("nk,nkd->nd", cof, words)), ("rotated16", mu + torch.einsum("nk,nkd->nd", cofr, wordsr)), ("pca16", mu + parts.sum(1))]:
        ok = [splice_at(items[n]["ids"], L, items[n]["spos"], Xh[n:n + 1])[0].argmax().item() in items[n]["answer_ids"] for n in range(N)]
        fid[rd] = sum(ok) / N
    ...
    for n in range(N):
        ...
        lp = torch.log_softmax(splice_at(items[n]["ids"], L, items[n]["spos"], xs), -1); a = items[n]["ans_tok"]
...
log(summ); record(f"e420_twohop_subject_{name}" + ("" if K == 16 else f"_k{K}"), res, summ)
