"""e362b: the e360b drift analysis for a model folder saved by e362 (argument: model tag; late window = last 60% of steps). Against the final checkpoint (step143000) and between
...
torch.set_num_threads(16); RES = os.environ.get("WDD_RESULTS", "/workspace/wdd/results"); import re; tag = sys.argv[1]; files = glob.glob(os.path.join(RES, f"e362_{tag}", "*.pt")); step = lambda f: int(re.match(r"step(\d+)", os.path.basename(f)).group(1))
...
late = [s for s in steps if 0.4 * max(steps) <= s < max(steps)]; dis = {}
for l in blocks:
    ds = torch.stack([vs_final[s][str(l)]["sig_change"] for s in late]).mean(0) if late else torch.zeros(fin["sig"][l].shape[0]); dw = torch.stack([vs_final[s][str(l)]["write_change"] for s in late]).mean(0) if late else torch.zeros(fin["sig"][l].shape[0])
...
os.makedirs(RES, exist_ok=True); json.dump(dict(out, _exp=f"e362b_drift_{tag}", _time=time.strftime("%Y-%m-%d %H:%M:%S")), open(os.path.join(RES, f"e362b_drift_{tag}.json"), "w"), indent=1)
