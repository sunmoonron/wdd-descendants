"""GPU/CPU job scheduler, version 3 (several GPUs: each GPU job is pinned by CUDA_VISIBLE_DEVICES to the device with room;
a '#gpus=2' tag reserves the need on two devices for models that must be split). Version 2 notes follow.
...
DEF = dict(MAXG=12, MAXC=24, SAFE=3, SCALE=0.7, NEED={"olmo1b": 26, "qwen05": 18, "pythia410": 12, "step": 12, "gpt2": 8, "smollm2": 7})
...
def gpus():
    try:
        ...
        return [(float(r.split(",")[0]) / 1024, float(r.split(",")[1]) / 1024) for r in rows]
    except Exception: return [(0.0, 40.0), (0.0, 40.0)]
def ngpu_of(cmd):
    m = re.search(r"#gpus=(\d+)", cmd); return int(m.group(1)) if m else 1
    ...
    for d in os.listdir("/proc"):
        ...
        try:
            ...
            if len(argv) >= 2 and os.path.basename(argv[0]).startswith("python") and re.match(r"e\d+[a-z]?_\S*\.py$", argv[1]) and os.readlink(f"/proc/{d}/cwd") == f"{W}/scripts":
                env = [e.decode(errors="ignore") for e in open(f"/proc/{d}/environ", "rb").read().split(b"\0")]
                cvd = next((e.split("=", 1)[1] for e in env if e.startswith("CUDA_VISIBLE_DEVICES=")), None)
                devs = [0] if cvd is None else [int(x) for x in cvd.split(",") if x.strip().isdigit()]
                found.append((int(d), "python " + " ".join(argv[1:]), "g" if devs else "c", devs))
        except Exception: pass
    return found
done = set()
...
c0 = conf()
for pid, cmd, k, devs in adopt():
    run.append(dict(p=None, pid=pid, cmd=cmd, k=k, dev=devs, need=need_of(cmd, 1, c0), log=logname(cmd), t0=time.time())); ev(f"ADOPT pid={pid} {k} dev={devs} {cmd}")
...
while True:
    ...
    run = alive
    G = gpus(); now = time.time(); eff = []
    for d, (used_d, tot_d) in enumerate(G):
        jd = [j for j in run if j["k"] == "g" and d in j.get("dev", [0])]
        dec = sum(j["need"] for j in jd); yng = sum(j["need"] for j in jd if now - j["t0"] < c.get("YOUNG", 150))
        eff.append(max(used_d + yng, c.get("FLOOR", 0.6) * dec))
    used = sum(u for u, _ in G); tot = sum(t for _, t in G); declared = sum(j["need"] for j in run if j["k"] == "g"); effective = sum(eff)
    ...
    for k in ("g", "c"):
        ...
        for arr, cmd in sorted(pend[k], key=lambda x: (prio_of(x[1], rules), x[0])):
            ...
            if k == "g":
                need = need_of(cmd, 1.5 ** retries.get(kk, 0), c); nd = ngpu_of(cmd)
                frees = sorted(((G[d][1] - eff[d] - c["SAFE"], d) for d in range(len(G))), reverse=True)
                pick = [d for f, d in frees[:nd] if need <= f]
                if ng < c["MAXG"] and len(pick) == nd and (retries.get(kk, 0) < 2 or ng == 0):
                    envd = ENV + f"export CUDA_VISIBLE_DEVICES={','.join(str(d) for d in sorted(pick))}; "
                    p = subprocess.Popen(["bash", "-c", envd + cmd.split("#")[0]], stdout=open(logname(cmd), "w"), stderr=subprocess.STDOUT); run.append(dict(p=p, pid=p.pid, cmd=cmd, k="g", dev=sorted(pick), need=need, log=logname(cmd), t0=time.time()))
                    for d in pick: eff[d] += need
                    declared += need; ng += 1; running_keys.add(kk); ev(f"START gpu{sorted(pick)} need={need:.0f} free={frees[0][0]:.0f} {cmd}"); continue
            else:
                if nc < c["MAXC"]:
