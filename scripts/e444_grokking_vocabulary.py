...
variant = sys.argv[1]
cfg = dict(seed=0, wd=1.0, randlab=False, freeze=None, steps=25000, every=250, lr=1e-3, frac=0.3)
...
res = dict(variant=variant, cfg=cfg, log=log_rows, final=fin, key_freqs=key, top_mlp_words=top.tolist(), top_word_uses=cnt[top].tolist(), top_word_purity=purity_top.tolist(),
...
summ = (f"{variant}: final train/test acc {fin['train_acc']:.2f}/{fin['test_acc']:.2f}; test acc >0.5 from step {gstep}; label-free own-over-rotation FVU advantage (k4, train) reaches half its max ({max(adv):.2f}) at step {half}, "
        ...
        f"emb top-5 Fourier share {fin['emb_top5_fourier']:.2f} | {time.time() - t0:.0f}s")
log(summ); record(f"e444_grok_{variant}", res, summ)
