...
def actual_components(n, L, mlp_only):
    """the 16 largest actual writes at item n's subject position among blocks 0..L: [16, D]"""
    mags = torch.cat([H[b][n].abs() * colnorm[b] for b in range(L + 1)]); top = mags.topk(64).indices
    DFF = colnorm[0].numel(); vecs = [H[int(t) // DFF][n][int(t) % DFF] * WD[int(t) // DFF][:, int(t) % DFF].float() for t in top]
    V = torch.stack(vecs)
    if not mlp_only:
        heads = []
        for b in range(L + 1):
            z = Z[b][n].view(arch.NH, arch.HD); Wo = WO[b].view(arch.D, arch.NH, arch.HD)
            heads.append(torch.einsum("hd,Dhd->hD", z, Wo))                                    # [NH, D] per-head writes
        V = torch.cat([V, torch.cat(heads), emb[tok_at[n]][None]])
...
for L in layers:
    ...
    act = torch.stack([actual_components(n, L, False) for n in range(N)]); gv = torch.stack([actual_components(n, L, True) for n in range(N)])
    S = dict(lens=lens_scores(X, gain, WU, rms), native16=pooled_word_scores(words, cof, gain, WU, rms),
             actual16=pooled_word_scores(act, torch.ones(N, 16, device=DEV), gain, WU, rms), geva16=pooled_word_scores(gv, torch.ones(N, 16, device=DEV), gain, WU, rms))
    ...
    log(f"L{L}: bridge@20 lens {r('lens', 'bridge@20'):.2f} native {r('native16', 'bridge@20'):.2f} actual {r('actual16', 'bridge@20'):.2f} geva {r('geva16', 'bridge@20'):.2f} | "
        f"country@1 lens {r('lens', 'country@1'):.2f} native {r('native16', 'country@1'):.2f} actual {r('actual16', 'country@1'):.2f} geva {r('geva16', 'country@1'):.2f}")
    del sel, words, act, gv, S
bl = lambda rd, m: max(res["readers"][f"{rd}@{L}"][m] for L in layers)
summ = (f"{name} n={N} subject position, best over blocks 10-20: bridge@20 lens {bl('lens', 'bridge@20'):.2f} native16 {bl('native16', 'bridge@20'):.2f} actual16 {bl('actual16', 'bridge@20'):.2f} geva16 {bl('geva16', 'bridge@20'):.2f} | "
        f"country@1 lens {bl('lens', 'country@1'):.2f} native16 {bl('native16', 'country@1'):.2f} actual16 {bl('actual16', 'country@1'):.2f} geva16 {bl('geva16', 'country@1'):.2f}")
log(summ); record(f"e421_subupdates_{name}", res, summ)
