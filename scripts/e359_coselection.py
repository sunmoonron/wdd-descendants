"""e359: co-selection, the post's own proposal, on the model's native writers at the final checkpoint. For every
...
tag = sys.argv[1]; c = Cache(tag); model, tok, fam = load_model(c.name); arch = Arch(model, fam); NB, NH = arch.NB, arch.NH; rng = random.Random(0); torch.manual_seed(0)
...
log(f"{tag} (units: {nh} heads + {len(nidx)} neurons): mean loss natural {res['mean_loss']['natural']:.2f}, induction {res['mean_loss']['induction']:.2f}, IOI {res['mean_loss']['ioi']:.2f} | " + " || ".join(f"{nm}: global mode share {v['all_units']['global_share']:.2f} (|corr with batch loss| {abs(v['loss_corr_with_global_mode']):.2f}), modes above MP edge {v['all_units']['above_mp']} / above shuffled null {v['all_units']['above_null']} (heads {v['heads_modes']['above_null']}, neurons {v['neuron_modes']['above_null']}); after removing the global mode: communities k {v['community_all']['k']}, modularity {v['community_all']['modularity']:.3f} vs shuffled {v['community_all']['modularity_shuffled']:.3f}; top-100 correlations same-layer {v['community_all']['top100_same_layer']:.2f} (chance {v['community_all']['same_layer_chance']:.2f}); induction-selected heads residual corr {v['induction_heads_residual_corr']:+.2f} vs random heads {v['random_heads_residual_corr']:+.2f}" for nm, v in (("natural", res["natural_only"]), ("mixed", res["mixed"]))))
os.makedirs(os.path.join(RESULTS, "e359"), exist_ok=True); torch.save(dict(SH=SH, SN_sel=SN[:, nidx], nidx=nidx, TY=TY, LO=LO, heads=heads, NB=NB, NH=NH, DFF=DFF), os.path.join(RESULTS, "e359", f"{tag}.pt"))
record(f"e359_cosel_{tag}", res, " || ".join(f"{nm}: global {v['all_units']['global_share']:.2f} modes>null {v['all_units']['above_null']} Q {v['community_all']['modularity']:.3f}/{v['community_all']['modularity_shuffled']:.3f} same-layer {v['community_all']['top100_same_layer']:.2f}/{v['community_all']['same_layer_chance']:.2f} ind-heads corr {v['induction_heads_residual_corr']:+.2f} vs {v['random_heads_residual_corr']:+.2f}" for nm, v in (("natural", res["natural_only"]), ("mixed", res["mixed"]))))
