...
Ronish Bhatt ([ORCID 0009-0000-8835-5380](https://orcid.org/0009-0000-8835-5380)), September 2026. Companion to [Weight-Dictionary Decomposition](https://github.com/sunmoonron/weight-dictionary-decomposition) (WDD), which reads a transformer's residual state as a sparse combination of the model's own write vectors. This repository holds the follow-on program: 469 experiments (e00 to e506 with variants) and about 2,000 recorded runs on five small models, run between 2026-09-19 and 2026-09-24, one to five minutes each on one GPU. Phase 1 (e00 to e356) follows one WDD write through the network and asks what it becomes, with the theory the results support and the literature they sit in; it uses forward passes, ablations, injections and closed-form fits only. Phase 2 (e357 to e373) tests the program's tools against known circuits, the co-selection proposal of the LessWrong post "What if not Circuits?", and per-neuron drift across Pythia and OLMo training. Phase 3 (e374 to e506) turns to readers and readouts and then to self-description: how well a model's own write rows describe its own states. It adds Qwen2.5-7B, and it trains toy transformers, grokking networks and a small sequence model from scratch, some with writer or reader rows frozen. **Start with [`docs/DIGEST.md`](docs/DIGEST.md)**, a ten-minute, theory-first summary that says which routes are closed and why; [`docs/ATLAS.md`](docs/ATLAS.md) organises all 469 experiments into twenty-two technical areas and shows how their results connect, and the table below summarises it.
...
| 22 | [The extreme-value model tested](docs/atlas/22_model_tested.md) | 3: e503, e505, e506, S64 | The floor stands (rotated calibration 0.97-1.00 in 27 cells); the numerator is neither the largest write's own coefficient (0.3-0.7 of a winning projection) nor the state cloud's second-order alignment (1.02-1.24 of the non-writers' 1.3-2.3 excess) but the position's chord of large writes, centred against the common direction they share, which predicts GPT-2's maximum within 13% and names the atom in a third to a half of states, with the crowd's contraction (OLMo) and the small writes (Pythia) as the corrections that matter; the accent is the dense sum on head atoms, the words the sparse chord |
...
This is the phase-1 map by rounds; [`docs/ATLAS.md`](docs/ATLAS.md) maps all 469 experiments by technical area. The program ran as rounds, each answering the previous round's open questions. The rounds in brief (script ranges are approximate; the narrative in `docs/FINDINGS.md` has every one):
