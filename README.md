...
Ronish Bhatt ([ORCID 0009-0000-8835-5380](https://orcid.org/0009-0000-8835-5380)), September 2026. Companion to [Weight-Dictionary Decomposition](https://github.com/sunmoonron/weight-dictionary-decomposition) (WDD), which reads a transformer's residual state as a sparse combination of the model's own write vectors. This repository holds the follow-on program: 481 experiments (e00 to e516 with variants) and about 2,000 recorded runs on five small models, run between 2026-09-19 and 2026-09-24, one to five minutes each on one GPU. Phase 1 (e00 to e356) follows one WDD write through the network and asks what it becomes, with the theory the results support and the literature they sit in; it uses forward passes, ablations, injections and closed-form fits only. Phase 2 (e357 to e373) tests the program's tools against known circuits, the co-selection proposal of the LessWrong post "What if not Circuits?", and per-neuron drift across Pythia and OLMo training. Phase 3 (e374 to e516) turns to readers and readouts and then to self-description: how well a model's own write rows describe its own states. It adds Qwen2.5-7B, and it trains toy transformers, grokking networks and a small sequence model from scratch, some with writer or reader rows frozen. **Start with [`docs/DIGEST.md`](docs/DIGEST.md)**, a ten-minute, theory-first summary that says which routes are closed and why; [`docs/ATLAS.md`](docs/ATLAS.md) organises all 481 experiments into twenty-three technical areas and shows how their results connect, and the table below summarises it.
...
| 17 | [WDD beyond description: forensics, steering, learning, grammar, self-consistency, interchange, life cycle](docs/atlas/17_forensic_instrument.md) | 16: e456-e471, S47-S54 | Self-description survives compression that breaks the model, so it does not detect damage. WDD names what a fine-tune changed but does not compress it. A native word steers as efficiently per unit of displacement as a dense vector. Its checksum, its ledger (for a state's future) and its forgery detection add nothing over simple baselines. It is a forward coordinate, not a learning one; its words have no grammar; what a description omits persists downstream. Across the band of blocks where a noun is written, a few native words are good interchange coordinates for it. Over training, becoming a word and becoming important are separate processes. |
| 18 | [WDD applied to itself: fixed points, word algebra, role typing, induced geometry, and their audits](docs/atlas/18_recursion_composition.md) | 6: e472-e477, S55-S56 | Applied to its own description, WDD is a projection: one application is a fixed point, for rotated words too. Native handles for two attributes of one word act independently and compose in Qwen (gender alone 92%, both from two words 74%), where rotated words fail and the task's principal directions do as well; the gender handle is one shared MLP row that works across words and languages (98%). Identity handles are not typed by grammatical role, and for an entity copied into the answer native words are no better than rotated ones. WDD's distances between states are a coarse version of activation distance; its one native-only signal, the concentration of deep descriptions, survives its audit and is half prominence. |
| 19 | [WDD as a workspace reader: WorkspaceBench's problems](docs/atlas/19_workspace_reader.md) | 6: e478-e482 with e478b, S57 | Against the benchmark's problems at 7B: the native reader almost never claims a wrong bridge country where the lens claims nothing, and no certificate helps where fabrication occurs (arithmetic); a concept held in mind is surfaced three times as often as by the lens, always as a computed MLP row and never as an echo; one native word carries a two-token name as a unit and disambiguates it (24 of 25) where the lens shows the second token in 4%; per-word reading loses half of a finished answer and reading the words as a whole recovers it; signs do not carry order. |
| 20 | [Calibration against classical theory](docs/atlas/20_classical_calibration.md) | 13: e483-e493, e498, e500, S58-S62 | The detection threshold is a Gumbel law whose two parameters predict the detection curve within 0.1 (resolution 0.25-0.4 of the state's norm); its scale is the atoms' second moment for a rotated dictionary and 2.9-6.2 times that for the native one, a provenance factor that is write sparsity on an accent: the largest writes standing out, rising through training, over a collective alignment that forms by Pythia step 512 and decays (Pythia and OLMo). The description is a non-redundant code with correction; its first word carries 0.4-1.2 bits about the next token; the rows are the heavy-tailed directions of the states, which FastICA partly rediscovers; provenance loss has one timescale; MLP rows are the words at every checkpoint. |
| 21 | [The provenance layer under SAEs](docs/atlas/21_sae_bridge.md) | 8: e494-e497, e499, e501, e502, e504, S59-S64 | A learned SAE feature's top native word is an MLP row for 76-93% of features at every depth of GPT-2, from any block; features are sparser in native words than in rotated ones but as sparse as a whole state, not a write (ReLU family), or between covariance-matched directions and states (TopK family); a quarter of the most used rows are features; a feature's top row separates the positions where it fires at AUC 0.62 (ReLU) and 0.73 (TopK), the eight-word ledger 0.61 and 0.83.; the TopK family's stronger row-following is its features' sparsity, not its parameterisation (e501); the other rows add modestly where features are sparse; on Pythia-160m the top row follows the feature but the features are not state-like |
| 22 | [The extreme-value model tested](docs/atlas/22_model_tested.md) | 8: e503, e505-e511, S64-S68 | The floor stands (rotated calibration 0.97-1.00 in 27 cells); the numerator is neither the largest write's own coefficient (0.3-0.7 of a winning projection) nor the state cloud's second-order alignment (1.02-1.24 of the non-writers' 1.3-2.3 excess) but the position's chord of large writes, centred against the common direction they share, which predicts GPT-2's maximum within 13% and names the atom in a third to a half of states, with the crowd's contraction (OLMo) and the small writes (Pythia) as the corrections that matter; the accent is the dense sum on head atoms, the words the sparse chord; against the whole profile the chord is a partial predictor, and the crowd amplifies it early in training and contracts it late; the clean maximum is selection over a nearly exchangeable residual (GPT-2), and the crowd's contraction is the MLPs' response to the chord, born with area 05's learned contraction; the contraction itself is isotropic and blind to the atoms, and the chord's sparing is a nonlinear, chord-amplitude effect specific to the position's own writes (a fake chord of native rows is contracted like a random direction) |
| 23 | [The onset of wordhood in training](docs/atlas/23_training_sweep.md) | 7: e512-e516, e515b, e516b, S68-S72 | At 18 Pythia checkpoints wordhood turns on in a fast phase (half the writer-win rate's rise between steps 2000 and 8000) and then creeps; sparsification precedes it, the contraction and the chord's growth follow; the accent is in place before step 512; the late attention phase erodes the last block's words; before it, the state cloud collapses to a dozen dimensions at steps 16-64 (the accent's birth) and sparsification begins as it re-expands, with the gradient spread, not concentrated; the future words have no lineage before sparsification, selectivity leading usage by a doubling from step 2000, and the early vocabulary is other rows; the vocabulary is replaced in identity at the churn rate of the large rows, under a boundary that drifts toward larger, sparser rows and then holds; the boundary is the extreme-value floor, which predicts the next checkpoint's entrants out of sample at 0.76-0.97 |
...
flowchart TD
  ...
  A01 --> A20["20 Classical calibration"]
  A14 --> A21["21 SAE bridge"]
...
| `scripts/e391_*.py` to `scripts/e516b_*.py`, `scripts/sd_common.py`, `ws_common.py`, `lr_common.py`, `ma_common.py`, `results/FINDINGS_box3.log` | Phase 3 continued: self-description and the native vocabulary (e391 to e414, e426 to e443), the Qwen2.5-7B workspace agenda and established lenses (e415 to e425), the huge directions and sinks (e432 to e443), the vision round with models trained from scratch (e444 to e450), causal tests of concept words and a re-implemented block (e451 to e455), WDD as a forensic instrument (e456 to e461), the learning signal in native coordinates (e462), a grammar over native words (e463 to e464), the self-consistency of descriptions (e465), systems-theory and causal-abstraction tests (e466 to e470), the life cycle of native words (e471), WDD applied to its own objects: fixed points, word algebra, role typing and induced geometry (e472 to e475), the audits of its two open results (e476 to e477), WDD put to WorkspaceBench's problems as a workspace reader (e478 to e482), the instrument calibrated against classical theory (e483 to e492), the provenance factor decomposed with a bridge to SAE features (e493 to e494), that bridge across depth and in activation with the vocabulary's two clocks (e495 to e496), the control that shows a feature is state-like, not write-like, in native words (e497), and the follow-ups: the two clocks' anatomy in Pythia and OLMo, a second SAE family, the kind of atom that becomes a word (e498 to e500), what predicts whether a feature follows its writers (e501), the extreme-value model tested state by state and corrected (e502 to e511), the onset of wordhood at eighteen checkpoints and its prehistory at thirteen (e512, e513), the lineage of the future words (e514), the vocabulary's turnover (e515, e515b), and what governs entry into it (e516, e516b). |
| `docs/DIGEST.md`, `docs/ATLAS.md`, `docs/atlas/` | The digest (theory first, ten minutes, with the routes the mathematics closes) and the atlas: all 481 experiments in twenty-three technical areas, one page per area, and `index.tsv` mapping every id to its area, session, status, script and results. |
...
This is the phase-1 map by rounds; [`docs/ATLAS.md`](docs/ATLAS.md) maps all 481 experiments by technical area. The program ran as rounds, each answering the previous round's open questions. The rounds in brief (script ranges are approximate; the narrative in `docs/FINDINGS.md` has every one):
...
## Native words as interchange coordinates, and a large Kalman gap (e466 to e469)

An eighth relayed review framed WDD in terms of systems theory (minimal realisation), predictive information and causal abstraction. Four tests ran in about four minutes of GPU time (details in `docs/FINDINGS.md`, session 52).

| Experiment | Question | Result |
| --- | --- | --- |
| e469 | Are native words interchange coordinates for a high-level variable (causal abstraction)? | Across blocks, yes. Setting a few native words of the base prompt's state to the source prompt's values at the noun, at every block up to the middle, carries the translated noun: one word per block switches 67% of Qwen's answers and four switch 96%. Rotated words manage 0-36% and the task's own principal directions 8-66% at the same size (SmolLM2: 22-65% against 1-28% and 4-38%). At a single block the principal directions win. Patching the whole state at that position and block switches 78-99%, which corrects session 46's explanation of weak single-depth effects. |
| e466 | How many extra dimensions make a 16-word description dynamically sufficient (the "Kalman gap")? | Many. With 64 extra directions of what the words leave out, 7-11% of the loss is still missing. The omitted content is spread over many dimensions. |
| e467 | What does the description leave out? | The same kinds of information it keeps: the current, previous and next token and the position are readable from both halves. |
| e468 | Does a description keep what later tokens read from a position? | Later tokens barely depend on one middle-depth state. The native description is best for the position itself; for later tokens it is best in Qwen and ties the principal components in GPT-2. |

## Where native words carry a variable (e470)

A ninth relayed review asked where and how e469's native-word handles work. One mapping experiment ran in about five minutes of GPU time (details in `docs/FINDINGS.md`, session 53).

| Question | Result |
| --- | --- |
| At which blocks does a native-word interchange work? | Only across the band of blocks in which the noun is written. In Qwen, four native words per block over blocks 0-4 switch 85% of answers and over 0-6 switch 94%. Starting from block 8 switches only 15%, although patching the whole state there still switches 100%. SmolLM2's band is about blocks 6-12. At a single block native words almost never work, while the whole state works from early blocks on. |
| Are the carrier words the same identities throughout? | Partly. Consecutive blocks share 41-47% of their top four carrier words, and a third to a half are written by the last two blocks: the variable is re-written by new writers as it goes. |
| Is the native advantage over task PCA about PCA being local to a block? | Partly. One PCA basis shared across blocks is as good as, or better than, per-block bases at 16 dimensions, so the variable's direction is largely shared across depth. Native words lead at small sizes in both models. |

## The life cycle of a native word (e471)

A tenth relayed review proposed that native words have a life cycle: a direction becomes a causal handle, then a word, then something readable, and words that die take their function with them. Its other proposals were already answered in the atlas (details in `docs/FINDINGS.md`, session 54).

| Question | Result |
| --- | --- |
| Over Pythia-410m's training (eight checkpoints), does a neuron become important before it becomes a native word, and does its function die when it falls out of use? | Neither. A neuron's usage in the model's own descriptions and its importance (the loss change when it is zeroed) evolve independently. Neither predicts the other's later change (Spearman -0.01 and +0.06), they do not move together (+0.04), and words that fall out of use keep their importance. Single-neuron importance is tiny and noisy, so a small effect could be hidden. The number of distinct words in use peaks at step 2000 (14,115) and falls to 9,435 by the end. |

## WDD applied to itself, word algebra, role typing and induced geometry (e472 to e475)

An eleventh relayed review proposed applying WDD recursively to its own objects, and a twelfth proposed treating it as a measurement language. Most of their proposals were already answered in the atlas or follow from OMP's properties; `docs/FINDINGS.md` (session 55) says which, and why. Four tests ran in about seven minutes of GPU time, including the two experiments deferred in sessions 53-54.

| Experiment | Question | Result |
| --- | --- | --- |
| e472 | Is the 16-word description a fixed point when WDD is applied to it again? | Yes, trivially. After one application 96-100% of descriptions choose the same words again, and loss and energy stop changing, for native and rotated words alike: the operator is a projection, with no drift, collapse or attractor. Native descriptions are only more stable to 5% noise (about 13 of 16 words kept, against 10 for rotated words). |
| e473 | Do native handles for two attributes of one word act independently and compose ("word algebra")? | In Qwen, yes. For kinship words carrying gender and generation (king, queen, prince, princess and similar, translated from French, Spanish and German), four native words per block for the gender difference change only the gender in 92% of items, and those for generation only the generation in 75%. Gender from one word and generation from another give the doubly changed word in 74% (83% with sixteen words), and the effects add within 24%. Rotated words fail at four words (6%, 25%, 16%); the task's own principal directions do as well as native words. SmolLM2 had too few correct items to test. |
| e474 | Are identity handles typed by grammatical role (subject against object)? | No. In "The dog chased the cat. Who chased?"-style questions, a handle found with the entity in the other role switches as many answers as the item's own, although the words shared between roles fall to about two thirds at the middle block. On this task native words are no better than rotated ones, so e469's native advantage depends on the variable. |
| e475 | Does WDD give its own distance between states, one that predicts behaviour beyond activation distance? | Barely. Activation distance predicts next-token similarity best. WDD coefficients or word sets add only +0.04 to +0.09 beyond it, nearly as much as rotated words do. One native-only signal remains: at the deepest depth, the shape of a description's coefficient profile, with no word identities, adds +0.13-0.14 beyond activation distance and the entropy gap in all three models. What it encodes is open. |

## Audits from the digest (e476, e477)

Writing `docs/DIGEST.md` left two results to check before they could be built on. Both ran on a new box in about eight minutes of GPU time (details in `docs/FINDINGS.md`, session 56).

| Experiment | Question | Result |
| --- | --- | --- |
| e476 | Is e475's native-only signal, the concentration of a deep description's coefficient profile, an artefact of the normalisation or of k, or a proxy for something plain? | Neither. It is the same under L1 normalisation and at 8 or 32 words, rotated profiles carry none of it, and the norm, position, entropy, top probability, token frequency, copying and token identity each remove at most 0.03 of it. A third to a half of it is how well sixteen own words fit and how much one own write dominates the state, which is prominence, a quantity a rotated dictionary cannot compute; +0.07 to +0.11 remains open. The dominating word is an MLP row in 86-98% of positions. In Qwen the PCA profile carries as much. |
| e477 | Are e473's attribute handles the item's or the vocabulary's? | Gender is the vocabulary's. Four native words found on the other kinship words, in the other two languages, change only the gender in 98% of Qwen's items, better than the item's own four (92%), and at sixteen words without leaking into generation (98% against 73%). One MLP row of block 4 sits in 91% of the items' own gender supports. Rotated words fitted to the same direction give 7% at four words. The generation handle is only half shared at four words (48% against 75%). |

## WDD against WorkspaceBench's problems (e478 to e482)

WorkspaceBench (LessWrong, 2026-09-23) scores activation-to-text readers on intermediates a model computes but never writes, and names the problems: single-token readers are a bag of words, expressive readers confabulate, and a reader can cheat by echoing the prompt. Sessions 40-42 had found the native-word reader is not a better reader in general. Session 57 asked whether what it has that no text generator has, provenance, a number per claim and separately readable parts, answers those problems (Qwen2.5-7B and its instruct variant; details in `docs/FINDINGS.md`, session 57).

| Problem | Experiment | Result |
| --- | --- | --- |
| Trust | e478, e478b | On two-hop bridges the native reader's country claims are right 73 of 73 at the subject token, where the lens claims nothing, and 110 of 111 at the end. On chained arithmetic no reader is reliable (precision 0.48-0.63 against a chance rate of 0.50) and no certificate, coefficient or causal receipt, separates fabricated digits from right ones. |
| Prompt echo | e479 | A concept the instruct model is told to hold in mind while copying a sentence is surfaced at the writing positions by native words in 30% of items (lens 10%), at 0.91 precision, always on MLP rows and never on the concept's token embedding; under "do not think" nothing surfaces. Provenance tells computed content from an echo. |
| Losing nothing the lens has | e480 | Read word by word, the 16-word description loses half of a finished answer (basic, multilingual and poetry families); read as a whole it keeps 0.85-1.00 of what the lens sees, and the union of the two readings keeps 0.80-0.90 within ten tokens. No reader sees a planned rhyme at the end of a couplet's first line at 7B. |
| Multi-token concepts | e481 | One native word, an MLP row, carries both tokens of a two-token country name in 31% of items and names the right second token in 24 of 25 cases; the plain lens shows the second token in 4%. |
| Order | e482 | Without a question, the signs of native words do not carry who did what to whom; order is readable only from position and recency, by any reader. |

## WDD calibrated against classical theory (e483 to e492)

Session 58 returned to the program's original mandate and read WDD as an instrument, calibrating it against theory older than OMP on the five original models: ten experiments, 58 short runs (details in `docs/FINDINGS.md`, session 58).

| Theory | Experiment | Result |
| --- | --- | --- |
| Extreme values (Fisher and Tippett 1928, Gumbel 1935) | e483, e492 | The largest competitor correlation is a Gumbel variable in all five models, its mean grows as sqrt(2 ln m), and its two fitted parameters predict the detection curve of an injected write within 0.1; the resolution is 0.25-0.4 of the state's norm. For a rotated dictionary the law's scale is exactly the atoms' second moment; for the native dictionary it is 2.9-6.2 times that. This provenance factor is 6-14 at the first block, falls with depth to 3.5-5 in four models, and rises through Pythia's training from 2.2-3.0 at step 1000. |
| The erasure channel (Shannon 1948) and mutual coherence (Tropp 2004) | e484, e490 | A native description is a non-redundant code: one or two erased words cost a tenth of its value, and a random half refitted reaches 0.59-0.80 of OMP's own eight. It has correction: refitting native words after erasure recovers 0.02-0.09 of loss recovered where refitting rotated words costs. The supports are equally near-orthogonal for both dictionaries, so the correction is in what the words fit, not their geometry. |
| Bits per word (Shannon 1948, Fano 1961) and the entropy of a stream (Shannon 1951) | e485, e487 | The first native word carries 0.4-1.2 bits about the next token (rotated 0.2-0.3, the current token 0.9-1.4) and 1.6-4.4 bits about the current token. The word stream has 0.2-0.9 bits of sequential structure per position, half of the text's; the rotated stream has almost no entropy. |
| Heaps' law (Herdan 1960) | e486 | The growth exponent of the vocabulary in use is the same with and without provenance (0.60-0.76 against 0.55-0.78); provenance concentrates usage on 2-5% of the words used. |
| Projection pursuit (Friedman and Tukey 1974) and independent components (Comon 1994) | e488, e489 | The rows are the heavy-tailed directions of the state cloud (kurtosis 2-10 times rotated at the 90th percentile; OMP's usage follows it), and FastICA on the states alone lands on them at two to three times the rotated rows' match, in part: its components are built from several rows. |
| Prony's method (1795) | e491 | Provenance loss has one timescale: a single exponential with a per-block retention of 0.71-0.88, the contraction's, fits the fading of a block's top write; a second mode carries nothing. |

## The provenance factor decomposed, and SAE features in native words (e493, e494)

Session 59 answered the one open question a relayed conversation landed on, and ran the one bridge it proposed that was not in the atlas (details in `docs/FINDINGS.md`, session 59).

| Question | Experiment | Result |
| --- | --- | --- |
| Where does the provenance factor come from? | e493 | Mostly write sparsity. At early and middle depth the largest competitor correlation is one of the position's 64 largest actual writes (or its token embedding) in 73-100% of positions, and the writers alone reproduce the factor; rows that did not write there sit 1.3-1.9 above the rotated level. Across Pythia's training the writers' part rises from 1.0-1.5 at step 1000 to 1.6-2.6, while the non-writers' part is already there at step 1000 and hardly grows at the middle depth: the vocabulary forms as the largest writes become prominent, on top of the per-block accent of e443. |
| Which weight rows write a learned SAE feature? | e494 | In GPT-2 (Bloom's residual SAE at the input of block 7), an MLP row is the top native word for 93% of features, from every block up to the sixth. Features are sparser in native words than in rotated ones (unexplained 0.71 against 0.91 at four words, 0.48 against 0.72 at sixteen) but not one row. A quarter of the most used native rows are features at cosine above 0.5; rotated rows never are. |

## The SAE bridge across depth and in activation, and the vocabulary's two clocks (e495, e496; e492, e493 on early checkpoints)

Session 60 ran the follow-ups of session 59 (details in `docs/FINDINGS.md`, session 60).

| Question | Experiment | Result |
| --- | --- | --- |
| Is the provenance layer under SAEs the same at every depth? | e495 | Yes, at five depths of GPT-2: sixteen native words leave 0.47-0.49 of a feature (rotated 0.71-0.73), the top word is an MLP row for 0.76-0.93 of features and comes from any block (the block just before the SAE holds 0.12-0.30), and a fifth to a quarter of the most used rows are features. The frequent features are the nearest to single rows. |
| Does a feature fire when its writer fires? | e496 | Partly. The top row's write size separates the positions where the feature fires from the rest at AUC 0.62 (a random row 0.51); the eight-word ledger, with no fitting, at 0.70; a quarter and a third of features follow them above 0.8. |
| When do the two components of the vocabulary form? | e492, e493 at Pythia steps 256-3000 | The accent (non-writing rows collectively aligned with the states) forms between steps 256 and 512 and plateaus; the words (the largest writes standing out) are below the rotated level at step 256, cross it near step 1000 and grow to the end. |

## The control the bridge needed (e497)

Under the native dictionary a random direction is 0.72 unexplained by sixteen words, which is also what the rotated dictionary leaves of anything; a direction drawn from the states' covariance is 0.63; an SAE feature is 0.48; a state is 0.47 (GPT-2, block 7 input). A feature is therefore exactly as sparse in native words as a whole state and far from a single write, with its top word an MLP row at the states' rate (0.93; random directions 0.54). The bridge of e494-e496 stands, reworded: a learned feature has a state's provenance profile, not a write's (details in `docs/FINDINGS.md`, session 61).

## The two clocks' anatomy, a second SAE family, and which atom becomes a word (e498 to e500)

Session 62 ran the follow-ups of the previous three (details in `docs/FINDINGS.md`, session 62).

| Question | Experiment | Result |
| --- | --- | --- |
| What carries each clock? | e498 (Pythia, 9 checkpoints; OLMo, 7) | Across 45 checkpoint-block cells of Pythia the writers' part of the provenance factor is write sparsity (Spearman +0.86 with the largest write's prominence, -0.81 with the effective number of writes, which falls from 6,800 to about 2,000 at block 12) and the non-writers' part is the second-order alignment of the state cloud with the atoms (+0.61); neither predicts the other at the next checkpoint. OLMo shows the same two clocks: write sparsity rising through 256,000 steps, the accent largest at the first checkpoint and decaying. |
| Does the SAE bridge hold on an independently trained family? | e499 (OpenAI TopK, 32k latents) | Provenance, yes: the top native word is an MLP row for 86-89% of features, and features are far sparser in native words than random or covariance-matched directions. State-likeness, partly: TopK features sit between covariance-matched directions and states. In activation, much stronger than on the ReLU family: a feature's top row separates the positions where it fires at AUC 0.73 and its eight-word ledger at 0.83, half of features above 0.8. |
| Which kind of atom becomes a word, and when? | e500 (Pythia, 9 checkpoints) | MLP rows, at every checkpoint: the MLP-only dictionary recovers within 0.04 of the full one from step 256 to the end. The head bases' share of the words peaks at steps 512-1000 and falls, while attention comes to carry 0.75 of the state's energy; the provenance advantage over rotation grows from 0.05-0.08 to 0.24-0.25. |
| Does the composition predict a ReLU feature's firing better than its top row? | e496 v3 | No, on the full sample: 0.59-0.63 against 0.62; the earlier 0.70 was a smaller sample. |

## What predicts whether a feature follows its writers, and the model written down (e501, THEORY.md 3m)

Feature by feature on the same states, the TopK family's stronger row-following (e499) is its features' sparsity: within both families the row-following AUC falls with the feature's frequency (Spearman -0.40 ReLU, -0.20 TopK) and rises with its activation strength and its cosine with its top native row; by shared frequency decile the families are alike at both ends (0.79 against 0.77 for the sparsest tenth, 0.53 against 0.51 for the densest); and the family label's effect on the AUC is +0.18 raw, -0.09 given frequency, -0.19 given all controls. Whether a feature follows its writers is a property of the feature, not of the SAE (e501).

`docs/THEORY.md` section 3m states the model the last sessions constrain: an atom is a word for a state when its projection stands above the extreme-value level of the competing atoms; a writer clears it through its coefficient (write sparsity), a non-writer through the second-order alignment of the state cloud with the atoms; the two have different histories in training; and because energy is a sum while wordhood is a maximum, the MLP rows are the vocabulary while attention comes to carry most of the state's energy.

## The model tested and corrected: the ledger under controls, another model, the chord and the crowd (e502 to e506)

Session 64 took the relayed queue as a researcher would. The ledger's advantage over the top row was put through the same controls as the top row (e502): the other native rows add modestly and where the feature is sparse (+0.10 in the sparsest ReLU decile, zero in the densest; +0.02 in TopK), random rows add nothing, the family gap is mostly frequency but TopK's ledger keeps a 0.1-0.25 lead at matched frequency in the middle deciles, and a feature's firing turns out to be a whole-state event in which the MLP writes are the smaller part (all MLP writes 0.77-0.84, the non-MLP part of the state 0.95-0.96). On the only public residual-stream SAE in the Pythia line (Pythia-160m, EleutherAI's TopK; e504) the top word is an MLP row and the top row follows the feature at the GPT-2 level, but the features are not state-like and the MLP writes as a whole carry nothing of the firing.

Then the extreme-value model of `docs/THEORY.md` section 3m was tested with nothing fitted (e503): the floor is right in every model, block and checkpoint (the rotated dictionary's maximum is 0.97-1.00 of the exact Gaussian extreme-value mean; 27 cells), and both terms of the numerator are wrong by 1.5-2. The non-writers' excess is a tail, not the state cloud's second-order alignment (a factor 1.02-1.24 of 1.3-2.3), and a writer's projection is 1.5-2.2 times its own coefficient. The anatomy (e505): a word's projection is its own coefficient (0.3-0.7) plus the position's other large writes through their cosines (0.3-0.7), less the crowd of small MLP writes (-0.05 to -0.5), the same crowd that erases writes in area 05; the top non-writer is the chord's shadow (GPT-2, OLMo) or attention's (Pythia); at Pythia step 512 the maxima are head atoms carried by the dense sum of small writes, and the sparse chord takes over by the end. The prediction from the ledger alone (e506) fails for the largest writes summed as written (twice the level where they are as large as the state, the wrong atom in 84-98% of states) because of the common direction they share, and succeeds once the chord is centred as the states are: within 1-13% of GPT-2's level at every block, the atom named in a third to a half of states; in OLMo the crowd's contraction and in Pythia the small writes are the corrections that matter. Section 3n of `docs/THEORY.md` records what stands (the floor, the two clocks, energy as a sum and wordhood as a maximum) and what replaces the numerator. Area 22 of the atlas holds these runs.

Against the whole projection profile rather than the maximum (e507), the centred chord is a partial predictor: in GPT-2 it is calibrated within 13-25% on the atoms it ranks highest and explains 54-66% of their variance, recovers the largest atom in 32-46% of states and a third of the top ten, and orders the true top hundred at 0.30-0.41; the crowd of small writes takes 0.16-0.31 of it and attention gives back 0.13-0.16. The centred sum of all writes does better in GPT-2 and worse in OLMo, where the crowd removes 0.42-0.46 of the chord. Over Pythia's training the crowd's gain along the chord runs from +1.37 at step 512 to -0.64 at the end, amplifying the large writes' directions early and cancelling them late, while attention's gain rises from 0.08 to 0.30-0.50; at step 512 the state is its MLP write-sum (R^2 0.98-0.99 for the whole profile).

Two follow-ups close the loop (e508, e509). The chord predicts the maximum better than the profile because the maximum is selected: in GPT-2 a null that keeps the chord and permutes the residual across atoms reproduces the maximum's level within 2%, the winner's chord rank within a hundred exactly and the winner's residual excess, and the chord's own top atom is calibrated (observed over predicted 0.94-1.01); in OLMo the residual contracts the chord's largest projections specifically (its top atom wins 0.13-0.19 of states against 0.60-0.79 under the null), and in Pythia's middle training it reinforces them. And the crowd's contraction of the chord is the network's response to the chord: removing the chord from what each MLP sees removes 1.2-1.9 times the crowd's gain along it; the response is born between Pythia steps 512 and 4000 and grows to the end, as area 05's learned contraction does; a random direction of the chord's size is contracted two to three times more than the chord, so the contraction spares the vocabulary; the early amplification is not a response but the anisotropy of the early state; and the rows that write a word come to read it (their response to the chord +0.3 to +0.45 at the end).

The contraction does not know the atoms (e510). As a linear response it is isotropic within 25% across native rows, rows used as words, rows never used, token embeddings, head bases, random, covariance-matched and principal directions, in all three models and at every Pythia checkpoint, with native rows contracted slightly more than random directions (1.01-1.17), and it is born isotropic between Pythia steps 512 and 4000, the MLP at step 512 amplifying on-manifold directions only. The chord's sparing is a nonlinear effect at chord amplitude: at a perturbation of the state's size the most-used rows are contracted a fifth to a third less than random directions in GPT-2 and at Pythia's block 18, and more in OLMo, the same model-by-model pattern as the residual's structure in e508. The sentence "the network learns to preserve its vocabulary" is not supported; the vocabulary is what a blind contraction leaves of a large, in-distribution pattern.

The sparing is the position's own writes (e511): a fake chord of native rows with the position's coefficients and the real chord's norm, whether random rows or the block's most-used, is contracted like a random direction in GPT-2 and at Pythia step 16000 and most of the way to it in OLMo and Pythia's end; only the rows that wrote at the position are spared. And at eighteen Pythia checkpoints (e512, atlas area 23) wordhood turns on as a fast crossover, not a discontinuity: half of the writer-win rate's rise falls between steps 2000 and 8000, the most concentrated change of nine observables; the effective number of writes falls first (midpoint near step 1500-1700), the crowd's gain along the chord and the linear contraction follow the wins (2400-15400), the chord's amplitude grows last (14000-19000), the accent is in place before step 512, and the state's late attention phase erodes the last block's words.

Below step 512 (e513, thirteen checkpoints from initialisation): the gradient concentrates transiently on a few MLP rows in the first 16 steps; the state cloud collapses to 9-16 effective dimensions between steps 16 and 64, attention's share of the state collapses with it and the accent is born (the non-writer tail factor 1.00 through step 32, 1.14-1.48 at step 64); a collapsed plateau holds to step 256; then sparsification begins as the cloud re-expands and attention takes the state back, every row turning selective (active at a fifth of positions instead of half, none dying) while the ledger's tails heavy and the contraction is born at warmup's end. The gradient is spread over two to three times more rows than at initialisation by then: sparsification is not a few rows being reinforced.

The rows that end as native words have no lineage before that (e514, fourteen checkpoints, 256 words per block against non-word rows of the same block matched on eventual magnitude): through step 1000 nothing measured tells them apart (activity, write kurtosis, gradient share, direction and magnitude all at AUC 0.42-0.58), while against random rows they are already the larger and more reinforced rows by step 1000 (AUC 0.64 and 0.59); from step 2000 their selectivity and kurtosis separate one doubling before their usage as words does, and the three grow together to the end, where a word is active at a tenth of positions against a quarter for a non-word of the same size; the gradient reinforces them no more than rows of their size and their directions settle no earlier. Before step 2000 the future words are used less than their controls: the early vocabulary is other rows, and turns over between steps 1000 and 8000. The same run closes a measurement trap in the two sweeps before it: the early writer wins were the token embedding, the maxima of steps 512-2000 are head atoms, and the rise from step 2000 is MLP writers.

The turnover itself, followed row by row (e515, e515b): the vocabulary is replaced in identity, a twentieth of the step-1000 words surviving to the end and seven-tenths of the end's words being new after step 4000, with even the last pair of checkpoints exchanging a quarter of the set; the rate of the turnover is the rate at which the 256 largest rows churn (retention within 0.15 at every pair), so it is not an event of the vocabulary's own; but the boundary moves, the rows entering the word set through steps 256-16000 being more selective and larger than the rows leaving it (AUC 0.66-0.76 and 0.76-0.95), and it is never a magnitude threshold (the words overlap the largest rows by 0.27 at most). A churning population under a wordhood boundary that drifts toward larger, sparser rows and then holds.

What that boundary is a threshold on (e516, e516b): the extreme-value floor. Ranked at a checkpoint by its largest projection over the calibrated Gumbel floor, a non-word row's chance of being a word at the next checkpoint is read at AUC 0.76 from step 1000 rising to 0.97 from step 16000, matching or beating its own current usage and far ahead of its magnitude, selectivity and kurtosis (0.5-0.7), with nothing fitted and nothing from the later checkpoint used. The projection that crosses carries the position's chord, since a row's own write clears the floor for only 0-1% of rows; the own write governs staying instead (stayers over leavers 0.90-0.99). The extreme-value criterion is the selection mechanism, not a description after the fact.
